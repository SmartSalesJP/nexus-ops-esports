...
- タスク画面の初期表示は「実行順」です。全担当者の次アクションから担当者を選ぶと、「今やる」「次にやる」「解除待ち」「完了ログ」を確認できます。「全タスク」へ切り替えると従来の進行表・検索・絞り込み・編集をそのまま利用できます。
- 実行順では大会運用サマリーを初期折りたたみにし、個人キューの「次にやる」は先頭5件から10件ずつ追加表示します。全タスク表示では従来どおりHero、KPI、Phase、週次状況を展開します。
- 実行順は保存済みの全タスクから毎回純粋導出し、画面の絞り込みに影響されません。期限帯、緊急度、状態、直接解除件数、期限日、Phase、IDの順で決定し、保留・未完了/欠落依存・循環依存は解除待ちへ分離します。
- `automationDisabled` は週次ルールによる再提案を止める設定であり、既存タスクを担当キュー、解除待ち、完了ログから隠しません。
- 実行順位、bucket、選択担当者、表示モードはschema、localStorage、JSON、Supabaseへ保存しません。状態変更だけが従来の検証・保存・read-back経路を通り、確認成功後に再順位付けされます。
