...
export interface CloudCacheEnvelope {schemaVersion:1;organizationId:string;stateVersion:number;verifiedAt:string;bundle:ExportBundle;workspaceConfig?:WorkspaceConfig|null}
...
export function writeCloudCache(organizationId:string,stateVersion:number,bundle:ExportBundle,workspaceConfig?:WorkspaceConfig|null,storage:StorageLike=localStorage):{ok:boolean;error?:string;value?:CloudCacheEnvelope}{
  const issues=validateBundle(bundle,workspaceConfig??undefined);if(issues.length)return{ok:false,error:`cache検証エラー: ${issues[0].path} ${issues[0].message}`}
  const value:CloudCacheEnvelope={schemaVersion:1,organizationId,stateVersion,verifiedAt:new Date().toISOString(),bundle,...(workspaceConfig?{workspaceConfig}:{})},serialized=JSON.stringify(value)
