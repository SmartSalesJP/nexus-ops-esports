...
import { people, statuses, urgencies, type Status, type Task, type ValidationIssue, type WorkspaceConfig } from '../types'
...
import { hasBoundedVisibleText,workspaceConfigFor } from '../workspace'

export type DueView=''|'soon'|'overdue'
type StatusChangeResult={ok:boolean;issues?:string[]}
type Props={tasks:Task[];taskResults?:TaskResultSheet[];config?:WorkspaceConfig|null;view:'kanban'|'list';search:string;department:string;status:string;phase:string;person:string;dueView:DueView;groupByTeam:boolean;readOnly?:boolean;busy?:boolean;setSearch:(value:string)=>void;setDepartment:(value:string)=>void;setStatus:(value:string)=>void;setPhase:(value:string)=>void;setPerson:(value:string)=>void;setDueView:(value:DueView)=>void;onAdd:()=>void;onSave?:(task:Task)=>Promise<ValidationIssue[]>;onDirty?:(dirty:boolean)=>void;onResult?:(task:Task)=>void;onDelete:(task:Task)=>void;onStatus:(id:string,status:Status,holdReason?:string)=>Promise<StatusChangeResult>}
...
function InlineTaskEditor({draft,baseline,focusField,saving,config,onChange,onSave,onDiscard}:{draft:Task;baseline:Task;focusField:string;saving:boolean;config:WorkspaceConfig;onChange:(task:Task)=>void;onSave:()=>Promise<ValidationIssue[]>;onDiscard:()=>void}){
  ...
  return <form ref={formRef} className="inline-task-form" aria-busy={saving} noValidate onSubmit={submit} onKeyDown={keyDown}>
...
function TaskCard({task,tasks,result,phase5,editing,draft,baseline,focusField,saving,textDirty,config,onStartEdit,onDraft,onSave,onDiscard,onResult,onDelete,onStatus,readOnly}:{task:Task;tasks:Task[];result?:TaskResultSheet;phase5:boolean;editing:boolean;draft?:Task;baseline?:Task;focusField:string;saving:boolean;textDirty:boolean;config:WorkspaceConfig;onStartEdit:(field:string)=>void;onDraft:(task:Task)=>void;onSave:()=>Promise<ValidationIssue[]>;onDiscard:()=>void;onResult:Props['onResult'];onDelete:Props['onDelete'];onStatus:Props['onStatus'];readOnly?:boolean}){
...
export function TaskBoard(props:Props){
  const {onDirty}=props,settings=workspaceConfigFor(props.config),phaseRefs=useRef<Array<HTMLButtonElement|null>>([]),searchRef=useRef<HTMLInputElement>(null),headingRef=useRef<HTMLHeadingElement>(null),restoreTriggerFocus=useRef<{taskId:string;field:string;filteredOut:boolean}|null>(null),query=props.search.trim().toLowerCase(),[session,setSession]=useState<{taskId:string;baseline:Task;draft:Task;focusField:string}|null>(null),[saving,setSaving]=useState(false),[focusNotice,setFocusNotice]=useState('')
  ...
  const phaseKeys=['',...settings.phases.map((item)=>String(item.code))],phaseLabel=(value:string)=>value===''?'全体':settings.phases.find((item)=>String(item.code)===value)?.name??`${settings.terminology.phase} ${value}`
  ...
  return <section aria-labelledby="tasks-title">
    ...
    <div className="filters" role="search">
      ...
      <select aria-label={`${settings.terminology.department}で絞り込み`} value={props.department} onChange={(event)=>props.setDepartment(event.target.value)}><option value="">{settings.departments.length}{settings.terminology.department}すべて</option>{settings.departments.map((unit)=><option value={unit.id} key={unit.id}>{unit.name}</option>)}</select>
    ...
    {filtered.length===0?<div className="empty"><Search/><h3>該当する{settings.terminology.task}がありません</h3></div>:props.groupByTeam?<div className="team-groups">{settings.departments.map((unit)=>{const items=filtered.filter((task)=>task.teamId===unit.id);if(!items.length)return null;const remaining=items.filter((task)=>task.status!=='完了').length,high=items.filter((task)=>task.urgency==='高'&&task.status!=='完了').length;return <section key={unit.id} className="team-group" aria-labelledby={`team-${unit.id}`}><header><div><span className="eyebrow">TEAM</span><h3 id={`team-${unit.id}`}>{unit.name}</h3></div><p>責任者 <b>{unit.owner||'未割当'}</b> · 未完了 {remaining}件 · 高緊急 {high}件</p></header>{renderCards(items)}</section>})}</div>:renderCards(filtered)}
  </section>
}
