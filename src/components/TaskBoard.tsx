...
type Props={tasks:Task[];taskResults?:TaskResultSheet[];view:'kanban'|'list';search:string;department:string;status:string;phase:string;person:string;dueView:DueView;groupByTeam:boolean;readOnly?:boolean;setSearch:(value:string)=>void;setDepartment:(value:string)=>void;setStatus:(value:string)=>void;setPhase:(value:string)=>void;setPerson:(value:string)=>void;setDueView:(value:DueView)=>void;onAdd:()=>void;onEdit:(task:Task)=>void;onResult?:(task:Task)=>void;onDelete:(task:Task)=>void;onStatus:(id:string,status:Status)=>void}
...
function StatusSelect({task,onStatus,readOnly,describedBy}:{task:Task;onStatus:Props['onStatus'];readOnly?:boolean;describedBy?:string}){return <select className="status-select" id={`status-card-${task.id}`} aria-label={`${task.title}のステータス`} aria-describedby={describedBy} value={task.status} disabled={readOnly} onChange={(event)=>onStatus(task.id,event.target.value as Status)}>{statuses.map((value)=><option key={value}>{value}</option>)}</select>}
function TaskCard({task,tasks,result,phase5,onEdit,onResult,onDelete,onStatus,readOnly}:{task:Task;tasks:Task[];result?:TaskResultSheet;phase5:boolean;onEdit:Props['onEdit'];onResult:Props['onResult'];onDelete:Props['onDelete'];onStatus:Props['onStatus'];readOnly?:boolean}){
  const due=deadlineState(task),blocked=isBlocked(task,tasks),milestone=isMilestoneChecklist(task),progress=checklistProgress(result?.checklistItems),completionIssues=milestoneCompletionIssues(task,result),completionWarningId=completionIssues.length?`milestone-completion-${task.id}`:undefined,resultLabel=milestone?`チェックリスト${progress.total?` ${progress.completed}/${progress.total}`:''}`:'成果シート'
  return <article className={`task-card urgency-${task.urgency} ${due.kind==='overdue'&&task.urgency==='高'?'critical-overdue':''} ${phase5?'phase5-check':''}`} data-task-id={task.id}>
    ...
    {completionWarningId&&<div id={completionWarningId} className={`milestone-completion-warning ${task.status==='完了'?'is-invalid-complete':''}`}><AlertTriangle size={14}/><span><b>{task.status==='完了'?'完了状態とチェックリストが不整合です。':'完了にはチェックリストの確認が必要です。'}</b> {completionIssues[0]}</span></div>}
    <div className="card-actions"><StatusSelect task={task} onStatus={onStatus} readOnly={readOnly} describedBy={completionWarningId}/>{onResult&&<button onClick={()=>onResult(task)} aria-label={`${task.title}の${resultLabel}`}><FileCheck2 size={16}/>{resultLabel}</button>}<button disabled={readOnly} onClick={()=>onEdit(task)} aria-label={`${task.title}を編集`}><Pencil size={16}/>編集</button><button onClick={()=>onDelete(task)} aria-label={`${task.title}を削除`} disabled={readOnly||authoritativeIds.has(task.id)} title={authoritativeIds.has(task.id)?'S4正本タスクは削除できません':'削除'}><Trash2 size={16}/>削除</button></div>
  </article>
}
...
export function TaskBoard(props:Props){
  ...
  const renderCards=(items:Task[])=>props.view==='kanban'?<div className={`task-grid ${props.phase==='5'?'phase5-grid':''}`}>{items.map((task)=><TaskCard key={task.id} task={task} tasks={props.tasks} result={props.taskResults?.find((item)=>item.taskId===task.id)} phase5={task.phase===5} onEdit={props.onEdit} onResult={props.onResult} onDelete={props.onDelete} onStatus={props.onStatus} readOnly={props.readOnly}/>)}</div>:<div className="task-list">{items.map((task)=><TaskCard key={task.id} task={task} tasks={props.tasks} result={props.taskResults?.find((item)=>item.taskId===task.id)} phase5={task.phase===5} onEdit={props.onEdit} onResult={props.onResult} onDelete={props.onDelete} onStatus={props.onStatus} readOnly={props.readOnly}/>)}</div>
