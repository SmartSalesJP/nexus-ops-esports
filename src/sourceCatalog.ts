...
export const sourceRef=(sourceId:SourceId,lineStart:number,lineEnd:number,confidence:SourceConfidence='high'):SourceRef=>{
  const {fileName,sha256,asOf}=SOURCE_CATALOG[sourceId]
  return {sourceId,fileName,sha256,lineStart,lineEnd,asOf,confidence}
}
