...
describe('task result sheet contract',()=>{
  const result=()=>({id:'task-result:P0-01' as const,taskId:'P0-01',resultBody:'完成',verificationState:'適合' as const,verificationSummary:'確認済み',verifiedBy:'reviewer',verifiedAt:'2026-08-17T00:00:00.000Z',deliverables:[{id:'deliverable:P0-01:1',title:'結果ファイル',type:'url' as const,href:'https://example.com/result',accessState:'利用可能' as const}],nextStep:'共有',completionCriteria:'承認',verificationMemo:'改ざんなし',updatedAt:'2026-08-17T00:00:00.000Z'})
  it('round-trips one result without changing canonical tasks',()=>{const source=bundle(),digest=createHash('sha256').update(JSON.stringify(source.tasks)).digest('hex'),candidate={...source,taskResults:[result()]};expect(saveBundle(candidate).ok).toBe(true);expect(readBundle().value.taskResults).toEqual(candidate.taskResults);expect(createHash('sha256').update(JSON.stringify(readBundle().value.tasks)).digest('hex')).toBe(digest)})
  it.each([0,1,3])('accepts and round-trips a result with %i HTTPS deliverable links',(count)=>{const item=result(),candidate={...bundle(),taskResults:[{...item,deliverables:Array.from({length:count},(_,index)=>({...item.deliverables[0],id:`deliverable:P0-01:${index+1}`,title:`成果${index+1}`,href:`https://example.com/result/${index+1}`}))}]};expect(validateBundle(candidate)).toEqual([]);expect(saveBundle(candidate).ok).toBe(true);expect(readBundle().value.taskResults?.[0].deliverables).toHaveLength(count)})
  it.each(['',' javascript:alert(1)','javascript:alert(1)','data:text/html,x','file:///tmp/a','//example.com','https:\\example.com','https://example.com/a b','https://user:pass@example.com','https://example.com:0/x','https://example.com:99999/x','https://例え.jp/x','https://[2001:db8::1]/x','https://example..com/x','https://127.1/x','https://example.com/\u0000x'])('rejects unsafe deliverable URL %s',(href)=>{const candidate={...bundle(),taskResults:[{...result(),deliverables:[{...result().deliverables[0],href}]}]};expect(validateBundle(candidate).some((issue)=>issue.path.endsWith('.href'))).toBe(true)})
  it.each(['https://example.com','https://sub-domain.example.com:443/path','https://192.168.1.10/result','https://xn--r8jz45g.jp/result'])('accepts the documented ASCII hostname and standard IPv4 URL policy %s',(href)=>{const candidate={...bundle(),taskResults:[{...result(),deliverables:[{...result().deliverables[0],href}]}]};expect(validateBundle(candidate)).toEqual([])})
  it('enforces the 100-character deliverable ID contract',()=>{const item=result(),candidate={...bundle(),taskResults:[{...item,deliverables:[{...item.deliverables[0],id:'x'.repeat(101)}]}]};expect(validateBundle(candidate).some((issue)=>issue.path.endsWith('.id'))).toBe(true)})
  it('rejects duplicate deliverables and orphan results',()=>{const first=result(),candidate={...bundle(),taskResults:[{...first,taskId:'missing',id:'task-result:missing' as const,deliverables:[first.deliverables[0],first.deliverables[0]]}]};const issues=validateBundle(candidate);expect(issues.some((issue)=>issue.path.endsWith('.taskId'))).toBe(true);expect(issues.some((issue)=>issue.path.endsWith('.deliverables'))).toBe(true)})
  it('round-trips ordered checklist items while old results remain compatible',()=>{const first=result(),items=[{id:'c2',title:'second',status:'進行中' as const,acceptanceCriteria:'criteria 2',assignee:'owner',reviewer:'',reviewedAt:'',evidenceMemo:'',holdReason:''},{id:'c1',title:'first',status:'完了' as const,acceptanceCriteria:'criteria 1',assignee:'owner',reviewer:'reviewer',reviewedAt:'2026-08-17T00:00:00.000Z',evidenceMemo:'evidence',holdReason:''}],candidate={...bundle(),taskResults:[{...first,checklistItems:items},first]};candidate.taskResults=[candidate.taskResults[0]];expect(validateBundle(candidate)).toEqual([]);expect(saveBundle(candidate).ok).toBe(true);expect(readBundle().value.taskResults?.[0].checklistItems?.map((item)=>item.id)).toEqual(['c2','c1']);expect(validateBundle({...bundle(),taskResults:[first]})).toEqual([])})
  it.each([
    [{status:'完了',reviewer:'',reviewedAt:'',evidenceMemo:''},'reviewer'],
    [{status:'保留',holdReason:' \u200b '},'holdReason'],
    [{status:'未着手',title:'   '},'title'],
  ])('rejects invalid checklist completion/hold/blank values %#',(override,path)=>{const first=result(),item=Object.assign({id:'c1',title:'item',status:'未着手' as const,acceptanceCriteria:'criteria',assignee:'',reviewer:'',reviewedAt:'',evidenceMemo:'',holdReason:''},override),candidate={...bundle(),taskResults:[{...first,checklistItems:[item]}]};expect(validateBundle(candidate).some((issue)=>issue.path.endsWith(`.${path}`))).toBe(true)})
  it('rejects checklist keys that the cloud SQL contract does not know',()=>{const first=result(),item={id:'c1',title:'item',status:'未着手' as const,acceptanceCriteria:'criteria',assignee:'',reviewer:'',reviewedAt:'',evidenceMemo:'',holdReason:'',unexpected:true},candidate={...bundle(),taskResults:[{...first,checklistItems:[item]}]};expect(validateBundle(candidate).some((issue)=>issue.path.endsWith('.checklistItems[0]')&&issue.message.includes('未対応'))).toBe(true)})
})
